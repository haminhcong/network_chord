...
Theo các định lý trong Chord protocol, thì trong thao tác **Key Insertion/Removal Operation**, hệ thống sẽ cần kết nối tới $O(log N)$ node trong trường hợp node kết nối với sensor là single node, và $O( \sum_{i=1}^{m}(log N_i))$ node trong trường hợp node kết nối với sensor là shared node.
